...
static float random_float(unsigned int* state, float min_value, float max_value) {
    const float unit = (float)(lcg_next(state) & 0x00FFFFFFu) / (float)0x01000000u;
...
Circle* generate_uniform_circles(size_t count, const BenchmarkConfig* config) {
    ...
    for (size_t i = 0; i < count; ++i) {
        circles[i].x = random_float(&rng, 0.0f, config->scene_width);
        circles[i].y = random_float(&rng, 0.0f, config->scene_height);
        circles[i].radius = random_float(&rng, config->min_radius, config->max_radius);
    }
...
Circle* generate_clustered_circles(size_t count, const BenchmarkConfig* config) {
    ...
    for (int i = 0; i < cluster_count && i < 8; ++i) {
        centers[i].x = random_float(&rng, config->scene_width * 0.15f, config->scene_width * 0.85f);
        centers[i].y = random_float(&rng, config->scene_height * 0.15f, config->scene_height * 0.85f);
        centers[i].radius = 0.0f;
    }

    for (size_t i = 0; i < count; ++i) {
        const int center_index = (int)(lcg_next(&rng) % (unsigned int)cluster_count);
        const float angle = random_float(&rng, 0.0f, 6.28318530718f);
        const float distance = random_float(&rng, 0.0f, config->cluster_spread);
